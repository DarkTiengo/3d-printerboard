...
export const en: Dicionario = {
  ...
  comum: {
    ...
    mostrar: (secao: string) => `Show ${secao}`,
    esconder: (secao: string) => `Hide ${secao}`,
  },
  ...
  impressora: {
    ...
    extrusora: 'EXTRUDER',
    extrudar: (mm: number) => `Extrude ${mm} mm of filament`,
    retrair: (mm: number) => `Retract ${mm} mm of filament`,
    quantidade: 'AMOUNT (MM)',
    quantidadeGrupo: 'Amount of filament in millimetres',
    velocidade: 'SPEED (MM/S)',
    velocidadeGrupo: 'Extrusion speed in millimetres per second',
    carregarFilamento: 'LOAD',
    descarregarFilamento: 'UNLOAD',
    semMacro: (macro: string) => `this machine has no ${macro} macro`,
    extrusaoImprimindo: 'pause the print to move filament',
    extrusaoFria: (min: number) => `the nozzle must be above ${min} °C`,
    falhaExtrusao: 'Could not extrude.',
    aquecimento: 'HEATING',
    graficoJanela: (min: number) => `last ${min} min`,
    graficoVazio: 'this machine keeps no temperature history',
    graficoFalhou: 'Could not read the temperature history.',
    graficoOffline: 'printer offline — the history lives on it',
    console: 'CONSOLE',
    consoleLinhas: (n: number) => `${n} ${n === 1 ? 'line' : 'lines'}`,
    consoleVazio: 'this machine has not said anything yet',
    consoleDica: 'type a G-code and press Enter',
    consoleEnviar: 'Send command',
    consoleOffline: 'printer offline — these are the last lines',
    consoleFalhou: 'Could not read the console.',
    consoleFalhouEnviar: 'Could not send the command.',
    macros: 'MACROS',
  ...
  alertas: {
    ...
    titulos: {
      ...
      falha_detectada: 'Possible print failure',
      impressora_offline: 'Printer unreachable',
      impressora_desligada: 'Printer powered off',
  ...
  deteccao: {
    titulo: 'FAILURE DETECTION',
    subtitulo: 'From the camera',
    configurar: 'Configure detection',
    vigiando: (n: number) => (n === 1 ? '1 machine watched' : `${n} machines watched`),
    nenhuma: 'no machine watched',
    indisponivel: 'Turned off on the server — set DETECCAO_ENABLED in .env and restart.',
    modelo: 'MODEL',
    modeloAusente: 'not on disk yet',
    modeloBaixando: 'downloading…',
    modeloPronto: (mb: string) => `ready — ${mb} MB`,
    modeloErro: (m: string) => `failed: ${m}`,
    semUrl: 'No DETECCAO_MODELO_URL set. Drop the .onnx file into data/modelos/ by hand.',
    baixar: 'Download the model',
    baixando: 'Downloading…',
    quais: 'ON WHICH MACHINES',
    semCamera: 'no camera configured',
    ligar: 'Watch',
    acao: 'ON CONFIRMATION',
    acoes: { alertar: 'Alert only', pausar: 'Pause', cancelar: 'Cancel' } as Record<string, string>,
    limiar: 'SENSITIVITY',
    limiarDica: 'Lower sees it sooner and is wrong more often. Blank uses the server default.',
    herda: (v: string) => `default (${v})`,
    comoFunciona: (n: number, s: number) =>
      `One frame every ${s} s, and it only acts after ${n} suspicious ones in a row — about ${Math.round((n * s) / 6) / 10} min of evidence.`,
    limites: 'It sees spaghetti, not under-extrusion or a layer shift. It is not a safety device.',
    salvar: 'Save',
    salvando: 'Saving…',
    salvo: 'Detection saved.',
    falhaSalvar: 'Could not save.',
    semPermissao: 'Only an administrator configures detection.'
  },
  gestao: {
    ...
    rotacao: 'CAMERA ROTATION',
    graus: (g: number) => `${g}°`,
    rotacaoDica: 'For a camera mounted sideways. Rotates the image in the dashboard; the frame saved with an alert stays as the camera sent it.',
