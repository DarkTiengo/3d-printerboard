...
export const fr: Dicionario = {
  ...
  comum: {
    ...
    mostrar: (secao: string) => `Afficher ${secao}`,
    esconder: (secao: string) => `Masquer ${secao}`,
  },
  ...
  impressora: {
    ...
    extrusora: 'EXTRUDEUR',
    extrudar: (mm: number) => `Extruder ${mm} mm de filament`,
    retrair: (mm: number) => `Rétracter ${mm} mm de filament`,
    quantidade: 'QUANTITÉ (MM)',
    quantidadeGrupo: 'Quantité de filament en millimètres',
    velocidade: 'VITESSE (MM/S)',
    velocidadeGrupo: 'Vitesse d’extrusion en millimètres par seconde',
    carregarFilamento: 'CHARGER',
    descarregarFilamento: 'DÉCHARGER',
    semMacro: (macro: string) => `cette machine n’a pas la macro ${macro}`,
    extrusaoImprimindo: 'mettez l’impression en pause pour bouger le filament',
    extrusaoFria: (min: number) => `la buse doit dépasser ${min} °C`,
    falhaExtrusao: 'Extrusion impossible.',
    aquecimento: 'CHAUFFE',
    graficoJanela: (min: number) => `${min} dernières min`,
    graficoVazio: 'cette machine ne garde pas d’historique de température',
    graficoFalhou: 'Impossible de lire l’historique de température.',
    graficoOffline: 'imprimante hors ligne — l’historique y reste',
    console: 'CONSOLE',
    consoleLinhas: (n: number) => `${n} ${n === 1 ? 'ligne' : 'lignes'}`,
    consoleVazio: 'cette machine n’a encore rien dit',
    consoleDica: 'tapez un G-code et appuyez sur Entrée',
    consoleEnviar: 'Envoyer la commande',
    consoleOffline: 'imprimante hors ligne — voici les dernières lignes',
    consoleFalhou: 'Impossible de lire la console.',
    consoleFalhouEnviar: 'Impossible d’envoyer la commande.',
    macros: 'MACROS',
  ...
  alertas: {
    ...
    titulos: {
      ...
      falha_detectada: 'Défaillance possible de l’impression',
      impressora_offline: 'Imprimante injoignable',
      impressora_desligada: 'Imprimante éteinte',
  ...
  deteccao: {
    titulo: 'DÉTECTION DE DÉFAILLANCE',
    subtitulo: 'Par la caméra',
    configurar: 'Configurer la détection',
    vigiando: (n: number) => (n === 1 ? '1 machine surveillée' : `${n} machines surveillées`),
    nenhuma: 'aucune machine surveillée',
    indisponivel: 'Désactivée sur le serveur — activez DETECCAO_ENABLED dans .env et redémarrez.',
    modelo: 'MODÈLE',
    modeloAusente: 'pas encore sur le disque',
    modeloBaixando: 'téléchargement…',
    modeloPronto: (mb: string) => `prêt — ${mb} Mo`,
    modeloErro: (m: string) => `échec : ${m}`,
    semUrl: 'DETECCAO_MODELO_URL non définie. Déposez le fichier .onnx dans data/modelos/ à la main.',
    baixar: 'Télécharger le modèle',
    baixando: 'Téléchargement…',
    quais: 'SUR QUELLES MACHINES',
    semCamera: 'aucune caméra configurée',
    ligar: 'Surveiller',
    acao: 'À LA CONFIRMATION',
    acoes: { alertar: 'Alerter seulement', pausar: 'Mettre en pause', cancelar: 'Annuler' } as Record<string, string>,
    limiar: 'SENSIBILITÉ',
    limiarDica: 'Plus bas voit plus tôt et se trompe plus. Vide utilise la valeur du serveur.',
    herda: (v: string) => `par défaut (${v})`,
    comoFunciona: (n: number, s: number) =>
      `Une image toutes les ${s} s, et n’agit qu’après ${n} soupçons d’affilée — environ ${Math.round((n * s) / 6) / 10} min de preuve.`,
    limites: 'Elle voit le spaghetti, pas la sous-extrusion ni le décalage de couche. Ce n’est pas un dispositif de sécurité.',
    salvar: 'Enregistrer',
    salvando: 'Enregistrement…',
    salvo: 'Détection enregistrée.',
    falhaSalvar: 'Enregistrement impossible.',
    semPermissao: 'Seul un administrateur configure la détection.'
  },
  gestao: {
    ...
    rotacao: 'ROTATION DE LA CAMÉRA',
    graus: (g: number) => `${g}°`,
    rotacaoDica: 'Pour une caméra montée de côté. Fait tourner l’image dans le tableau de bord ; l’image enregistrée avec une alerte reste telle que la caméra l’a envoyée.',
