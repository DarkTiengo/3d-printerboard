...
export const it: Dicionario = {
  ...
  comum: {
    ...
    mostrar: (secao: string) => `Mostra ${secao}`,
    esconder: (secao: string) => `Nascondi ${secao}`,
  },
  ...
  impressora: {
    ...
    extrusora: 'ESTRUSORE',
    extrudar: (mm: number) => `Estrudere ${mm} mm di filamento`,
    retrair: (mm: number) => `Ritrarre ${mm} mm di filamento`,
    quantidade: 'QUANTITÀ (MM)',
    quantidadeGrupo: 'Quantità di filamento in millimetri',
    velocidade: 'VELOCITÀ (MM/S)',
    velocidadeGrupo: 'Velocità di estrusione in millimetri al secondo',
    carregarFilamento: 'CARICA',
    descarregarFilamento: 'SCARICA',
    semMacro: (macro: string) => `questa macchina non ha la macro ${macro}`,
    extrusaoImprimindo: 'metti in pausa la stampa per muovere il filamento',
    extrusaoFria: (min: number) => `l’ugello deve superare i ${min} °C`,
    falhaExtrusao: 'Estrusione non riuscita.',
    aquecimento: 'RISCALDAMENTO',
    graficoJanela: (min: number) => `ultimi ${min} min`,
    graficoVazio: 'questa macchina non conserva lo storico di temperatura',
    graficoFalhou: 'Impossibile leggere lo storico di temperatura.',
    graficoOffline: 'stampante offline — lo storico resta su di lei',
    console: 'CONSOLE',
    consoleLinhas: (n: number) => `${n} ${n === 1 ? 'riga' : 'righe'}`,
    consoleVazio: 'questa macchina non ha ancora detto nulla',
    consoleDica: 'scrivi un G-code e premi Invio',
    consoleEnviar: 'Invia comando',
    consoleOffline: 'stampante offline — queste sono le ultime righe',
    consoleFalhou: 'Impossibile leggere la console.',
    consoleFalhouEnviar: 'Impossibile inviare il comando.',
    macros: 'MACRO',
  ...
  alertas: {
    ...
    titulos: {
      ...
      falha_detectada: 'Possibile guasto di stampa',
  ...
  deteccao: {
    titulo: 'RILEVAMENTO GUASTI',
    subtitulo: 'Dalla telecamera',
    configurar: 'Configura il rilevamento',
    vigiando: (n: number) => (n === 1 ? '1 macchina sorvegliata' : `${n} macchine sorvegliate`),
    nenhuma: 'nessuna macchina sorvegliata',
    indisponivel: 'Disattivato sul server — attiva DETECCAO_ENABLED in .env e riavvia.',
    modelo: 'MODELLO',
    modeloAusente: 'non ancora sul disco',
    modeloBaixando: 'download in corso…',
    modeloPronto: (mb: string) => `pronto — ${mb} MB`,
    modeloErro: (m: string) => `non riuscito: ${m}`,
    semUrl: 'DETECCAO_MODELO_URL non impostata. Metti il file .onnx in data/modelos/ a mano.',
    baixar: 'Scarica il modello',
    baixando: 'Download…',
    quais: 'SU QUALI MACCHINE',
    semCamera: 'nessuna telecamera configurata',
    ligar: 'Sorveglia',
    acao: 'ALLA CONFERMA',
    acoes: { alertar: 'Solo avvisare', pausar: 'Mettere in pausa', cancelar: 'Annullare' } as Record<string, string>,
    limiar: 'SENSIBILITÀ',
    limiarDica: 'Più bassa vede prima e sbaglia di più. Vuoto usa il valore del server.',
    herda: (v: string) => `predefinito (${v})`,
    comoFunciona: (n: number, s: number) =>
      `Un fotogramma ogni ${s} s, e agisce solo dopo ${n} sospetti di fila — circa ${Math.round((n * s) / 6) / 10} min di prove.`,
    limites: 'Vede lo spaghetto, non la sotto-estrusione né lo scostamento di strato. Non è un dispositivo di sicurezza.',
    salvar: 'Salva',
    salvando: 'Salvataggio…',
    salvo: 'Rilevamento salvato.',
    falhaSalvar: 'Impossibile salvare.',
    semPermissao: 'Solo un amministratore configura il rilevamento.'
  },
  gestao: {
    ...
    rotacao: 'ROTAZIONE DELLA TELECAMERA',
    graus: (g: number) => `${g}°`,
    rotacaoDica: 'Per la telecamera montata di lato. Ruota l’immagine nel pannello; il fotogramma salvato con un avviso resta come l’ha inviato la telecamera.',
