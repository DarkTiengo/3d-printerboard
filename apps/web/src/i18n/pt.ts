...
export const pt = {
  ...
  comum: {
    ...
    esconder: (secao: string) => `Esconder ${secao}`,
  },
  ...
  impressora: {
    ...
    velocidade: 'VELOCIDADE (MM/S)',
    velocidadeGrupo: 'Velocidade da extrusão em milímetros por segundo',
    carregarFilamento: 'CARREGAR',
    descarregarFilamento: 'DESCARREGAR',
    semMacro: (macro: string) => `esta máquina não tem a macro ${macro}`,
    ...
    graficoVazio: 'esta máquina não guarda histórico de temperatura',
    graficoFalhou: 'Não foi possível ler o histórico de temperatura.',
    graficoOffline: 'impressora offline — o histórico fica nela',
    console: 'CONSOLE',
    consoleLinhas: (n: number) => `${n} ${n === 1 ? 'linha' : 'linhas'}`,
    consoleVazio: 'esta máquina ainda não disse nada',
    consoleDica: 'digite um G-code e tecle Enter',
    consoleEnviar: 'Enviar comando',
    consoleOffline: 'impressora offline — estas são as últimas linhas',
    consoleFalhou: 'Não foi possível ler o console.',
    consoleFalhouEnviar: 'Não foi possível enviar o comando.',
    macros: 'MACROS',
  ...
  alertas: {
    ...
    titulos: {
      ...
      falha_detectada: 'Possível falha na impressão',
  ...
  deteccao: {
    titulo: 'DETECÇÃO DE FALHA',
    subtitulo: 'Pela câmera',
    configurar: 'Configurar a detecção',
    vigiando: (n: number) => (n === 1 ? '1 máquina vigiada' : `${n} máquinas vigiadas`),
    nenhuma: 'nenhuma máquina vigiada',
    indisponivel: 'Desligada no servidor — ligue DETECCAO_ENABLED no .env e reinicie.',
    modelo: 'MODELO',
    modeloAusente: 'ainda não está no disco',
    modeloBaixando: 'baixando…',
    modeloPronto: (mb: string) => `pronto — ${mb} MB`,
    modeloErro: (m: string) => `falhou: ${m}`,
    semUrl: 'Sem DETECCAO_MODELO_URL configurada. Ponha o arquivo .onnx em data/modelos/ na mão.',
    baixar: 'Baixar o modelo',
    baixando: 'Baixando…',
    quais: 'EM QUAIS MÁQUINAS',
    semCamera: 'sem câmera cadastrada',
    ligar: 'Vigiar',
    acao: 'AO CONFIRMAR',
    acoes: { alertar: 'Só avisar', pausar: 'Pausar', cancelar: 'Cancelar' } as Record<string, string>,
    limiar: 'SENSIBILIDADE',
    limiarDica: 'Mais baixo enxerga mais cedo e erra mais. Vazio usa o padrão do servidor.',
    herda: (v: string) => `padrão (${v})`,
    comoFunciona: (n: number, s: number) =>
      `Um quadro a cada ${s} s, e só age depois de ${n} suspeitas seguidas — cerca de ${Math.round((n * s) / 6) / 10} min de evidência.`,
    limites: 'Vê espaguete, não sub-extrusão nem camada deslocada. Não é dispositivo de segurança.',
    salvar: 'Salvar',
    salvando: 'Salvando…',
    salvo: 'Detecção salva.',
    falhaSalvar: 'Não foi possível salvar.',
    semPermissao: 'Só um administrador configura a detecção.'
  },
  gestao: {
