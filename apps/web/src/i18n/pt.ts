...
export const pt = {
  ...
  impressora: {
    ...
    extrusora: 'EXTRUSORA',
    extrudar: (mm: number) => `Extrudar ${mm} mm de filamento`,
    retrair: (mm: number) => `Retrair ${mm} mm de filamento`,
    quantidade: 'QUANTIDADE (MM)',
    quantidadeGrupo: 'Quantidade de filamento em milímetros',
    velocidadeExtrusao: (mms: number) => `a ${mms} mm/s`,
    extrusaoImprimindo: 'pause a impressão para mexer no filamento',
    extrusaoFria: (min: number) => `o bico precisa passar de ${min} °C`,
    falhaExtrusao: 'Não foi possível extrudar.',
    macros: 'MACROS',
  ...
  gestao: {
    ...
    rotacao: 'ROTAÇÃO DA CÂMERA',
    graus: (g: number) => `${g}°`,
    rotacaoDica: 'Para a câmera montada de lado. Gira a imagem no painel; o quadro guardado com um alerta continua como a câmera mandou.',
