...
export const es: Dicionario = {
  ...
  comum: {
    ...
    mostrar: (secao: string) => `Mostrar ${secao}`,
    esconder: (secao: string) => `Ocultar ${secao}`,
  },
  ...
  impressora: {
    ...
    extrusora: 'EXTRUSOR',
    extrudar: (mm: number) => `Extruir ${mm} mm de filamento`,
    retrair: (mm: number) => `Retraer ${mm} mm de filamento`,
    quantidade: 'CANTIDAD (MM)',
    quantidadeGrupo: 'Cantidad de filamento en milímetros',
    velocidade: 'VELOCIDAD (MM/S)',
    velocidadeGrupo: 'Velocidad de extrusión en milímetros por segundo',
    carregarFilamento: 'CARGAR',
    descarregarFilamento: 'DESCARGAR',
    semMacro: (macro: string) => `esta máquina no tiene la macro ${macro}`,
    extrusaoImprimindo: 'pausa la impresión para mover el filamento',
    extrusaoFria: (min: number) => `la boquilla debe pasar de ${min} °C`,
    falhaExtrusao: 'No se pudo extruir.',
    aquecimento: 'CALENTAMIENTO',
    graficoJanela: (min: number) => `últimos ${min} min`,
    graficoVazio: 'esta máquina no guarda historial de temperatura',
    graficoFalhou: 'No se pudo leer el historial de temperatura.',
    graficoOffline: 'impresora offline — el historial está en ella',
    console: 'CONSOLA',
    consoleLinhas: (n: number) => `${n} ${n === 1 ? 'línea' : 'líneas'}`,
    consoleVazio: 'esta máquina aún no ha dicho nada',
    consoleDica: 'escribe un G-code y pulsa Enter',
    consoleEnviar: 'Enviar comando',
    consoleOffline: 'impresora offline — estas son las últimas líneas',
    consoleFalhou: 'No se pudo leer la consola.',
    consoleFalhouEnviar: 'No se pudo enviar el comando.',
    macros: 'MACROS',
  ...
  alertas: {
    ...
    titulos: {
      ...
      falha_detectada: 'Posible fallo de impresión',
      impressora_offline: 'Impresora inaccesible',
      impressora_desligada: 'Impresora apagada',
  ...
  deteccao: {
    titulo: 'DETECCIÓN DE FALLOS',
    subtitulo: 'Por la cámara',
    configurar: 'Configurar la detección',
    vigiando: (n: number) => (n === 1 ? '1 máquina vigilada' : `${n} máquinas vigiladas`),
    nenhuma: 'ninguna máquina vigilada',
    indisponivel: 'Desactivada en el servidor — activa DETECCAO_ENABLED en .env y reinicia.',
    modelo: 'MODELO',
    modeloAusente: 'aún no está en el disco',
    modeloBaixando: 'descargando…',
    modeloPronto: (mb: string) => `listo — ${mb} MB`,
    modeloErro: (m: string) => `falló: ${m}`,
    semUrl: 'Sin DETECCAO_MODELO_URL configurada. Copia el archivo .onnx en data/modelos/ a mano.',
    baixar: 'Descargar el modelo',
    baixando: 'Descargando…',
    quais: 'EN QUÉ MÁQUINAS',
    semCamera: 'sin cámara configurada',
    ligar: 'Vigilar',
    acao: 'AL CONFIRMAR',
    acoes: { alertar: 'Solo avisar', pausar: 'Pausar', cancelar: 'Cancelar' } as Record<string, string>,
    limiar: 'SENSIBILIDAD',
    limiarDica: 'Más bajo lo ve antes y se equivoca más. Vacío usa el valor del servidor.',
    herda: (v: string) => `por defecto (${v})`,
    comoFunciona: (n: number, s: number) =>
      `Un fotograma cada ${s} s, y solo actúa tras ${n} sospechas seguidas — unos ${Math.round((n * s) / 6) / 10} min de evidencia.`,
    limites: 'Ve espagueti, no subextrusión ni capa desplazada. No es un dispositivo de seguridad.',
    salvar: 'Guardar',
    salvando: 'Guardando…',
    salvo: 'Detección guardada.',
    falhaSalvar: 'No se pudo guardar.',
    semPermissao: 'Solo un administrador configura la detección.'
  },
  gestao: {
    ...
    rotacao: 'ROTACIÓN DE LA CÁMARA',
    graus: (g: number) => `${g}°`,
    rotacaoDica: 'Para la cámara montada de lado. Gira la imagen en el panel; el fotograma guardado con una alerta sigue como lo envió la cámara.',
